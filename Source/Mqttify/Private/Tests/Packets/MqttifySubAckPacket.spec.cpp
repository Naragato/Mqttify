...
namespace Mqttify
{
	BEGIN_DEFINE_SPEC(
		...
		const TArray<uint8> Mqtt3SubAck = {
			// Fixed header
			0x90, // Packet type (0x90 = SUBACK)
			0x03, // Remaining Length (2 bytes for message ID, 1 byte for return code)
		...
		const TArray<uint8> Mqtt5SubAckNoProperties = {
			// Fixed header
			0x90, // Packet type (0x90 = SUBACK)
			0x04, // Remaining Length (2 bytes for message ID)
	...
	{
		Describe("TMqttifySubAckPacket<EMqttifyProtocolVersion::Mqtt_5>",
				[this]
				{
					It(
					...
					for (const EMqttifyReasonCode ReasonCode : ReasonCodes)
					{
						...
						It(FString::Printf(TEXT("Should deserialize with %s with properties"),
							...
							[this, ReasonCode]
							{
								...
								const TMqttifySubAckPacket<EMqttifyProtocolVersion::Mqtt_5>
									SubAckPacket(Reader, Header);
								const TTuple<FString, FString> PropertyData = MakeTuple(TEXT("test"), TEXT("test"));
								const FMqttifyProperties Properties{
									{
										FMqttifyProperty::Create<EMqttifyPropertyIdentifier::UserProperty>(
								...
								TestEqual(TEXT("Packet Identifier should be equal"),
										SubAckPacket.GetPacketIdentifier(),
										1);
								...
								TestEqual(TEXT("Properties should be equal"),
										SubAckPacket.GetProperties(),
										Properties);
							});
					}
				});

		Describe("TMqttifySubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>",
				[this]
				{
					It("Should serialize success",
						[this]
						{
							auto SubAckPacket = TMqttifySubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>{
								1, {EMqttifySubscribeReturnCode::SuccessQualityOfService0}
							};
					...
					It("Should Deserialize success",
						[this]
						{
							...
							const FMqttifyFixedHeader Header = FMqttifyFixedHeader::Create(Reader);
							const TMqttifySubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>
								SubAckPacket(Reader, Header);
