...
void FMqttifyClientConnectionTests::Define()
{
	BeforeEach(
		[this]()
		{
			...
			int32 ReturnCode;
			FString OutString;
			FPlatformProcess::ExecProcess(TEXT("docker"), TEXT("--version"), &ReturnCode, &OutString, nullptr);
			...
			const FString RunParams = FString::Printf(
				TEXT(
					"run -p %d:8080 -e \"DOCKER_VERNEMQ_ACCEPT_EULA=yes\" -e \"DOCKER_VERNEMQ_LOG__CONSOLE__LEVEL=debug\" -e \"DOCKER_VERNEMQ_ALLOW_ANONYMOUS=on\" --name %s -d vernemq/vernemq"),
				BrokerPort,
				*DockerContainerName);

			FPlatformProcess::ExecProcess(TEXT("docker"), *RunParams, &ReturnCode, &OutString, nullptr);
			...
			GConfig->GetBool(
				TEXT("LwsWebSocket"),
				TEXT("bDisableCertValidation"),
				bOriginalDisableCertValidation,
				GEngineIni);
	...
	LatentIt(
		...
		[this](const FDoneDelegate& TestDone)
		{
			...
			const FMqttifyConnectionSettingsBuilder ConnectionSettingsBuilder = FMqttifyConnectionSettingsBuilder(
				BrokerURL);
...
bool FMqttifyClientConnectionTests::IsBrokerReady(const int32 Port, const float TimeoutSeconds) const
{
	...
	int32 ReturnCode;
	FString OutString;
	FPlatformProcess::ExecProcess(TEXT("docker"), *RunningParams, &ReturnCode, &OutString, nullptr);
...
void FMqttifyClientConnectionTests::RemoveExistingContainer()
{
	...
	FPlatformProcess::ExecProcess(TEXT("docker"), *StopParams, &ReturnCode, &OutString, nullptr);
	if (ReturnCode != 0)
	{
	...
	FPlatformProcess::ExecProcess(TEXT("docker"), *RemoveParams, &ReturnCode, &OutString, nullptr);
	if (ReturnCode != 0)
	{
...
FString FMqttifyClientConnectionTests::GetDockerLogs() const
{
	...
	FPlatformProcess::ExecProcess(TEXT("docker"), *LogsParams, &ReturnCode, &OutString, nullptr);
	OutString.TrimStartAndEndInline();
