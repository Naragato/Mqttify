...
namespace Mqttify
{
	BEGIN_DEFINE_SPEC(MqttifyConnAckPacket,
					"Mqttify.Automation.MqttifyConnAckPacket",
					EAutomationTestFlags::ApplicationContextMask | EAutomationTestFlags::ProductFilter)

		static TArray<uint8> SerializeConnackMqtt3(const bool bSessionPresent,
													const EMqttifyConnectReturnCode ReturnCode)
		{
			const uint8 Flags = (bSessionPresent ? 0x01 : 0x00);
			TArray<uint8> ConnackPacket = {
				0x20, // Packet type (0x20 = CONNACK)
				0x02, // Remaining Length (2 bytes)
				Flags, // Session present flag
		...
		const TArray<uint8> InvalidRemainingLengthConnackMqtt3Packet = {
			...
			0x09, // Incorrect Remaining Length (it should be 2 bytes)
			0x09, // Invalid session present flag (should be either 0 or 1)
		...
		static TArray<uint8> SerializeConnackMqtt5(const bool bSessionPresent, EMqttifyReasonCode ReasonCode)
		{
			...
			TArray<uint8> ConnackPacket = {
				0x20, // Packet type
				3, // Size of Flags and Reason Code plus 1 byte for Properties Length of 0
				Flags, // Session present flag
				static_cast<uint8>(ReasonCode), // Reason code
				0 // Properties Length
			};
			return ConnackPacket;
		}

		static TArray<uint8> SerializeConnackMqtt5(const bool bSessionPresent,
		...
		TArray<FMqttifyProperty> ValidProperties = {
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::UserProperty>(
				MakeTuple(FString(TEXT("test")), FString(TEXT("test")))),
			...
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::MaximumQoS>(
				static_cast<uint8>(EMqttifyQualityOfService::AtLeastOnce)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::MaximumQoS>(
				static_cast<uint8>(EMqttifyQualityOfService::AtMostOnce)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::MaximumQoS>(
				static_cast<uint8>(EMqttifyQualityOfService::ExactlyOnce)),
			...
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::WildcardSubscriptionAvailable>(
				static_cast<uint8>(true)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::WildcardSubscriptionAvailable>(
				static_cast<uint8>(false)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::SubscriptionIdentifierAvailable>(
				static_cast<uint8>(true)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::SubscriptionIdentifierAvailable>(
				static_cast<uint8>(false)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::SharedSubscriptionAvailable>(static_cast<uint8>(true)),
			FMqttifyProperty::Create<EMqttifyPropertyIdentifier::SharedSubscriptionAvailable>(
				static_cast<uint8>(false)),
	...
	{
		Describe("TMqttifyConnAckPacket<EMqttifyProtocolVersion::Mqtt_5>",
				[this]
				{
						...
						for (const EMqttifyReasonCode ReasonCode : ValidReasonCodes)
						{
							It(FString::Printf(
								...
								[this, ReasonCode, bIsSessionPresent]
								{
									...
									TestPacketsEqual(TEXT("Packet should match expected"),
													ConnAckPacket,
													Expected,
													this);
								});

							It(FString::Printf(TEXT("Should deserialize with: session present %s, reason code %s"),
												bIsSessionPresent ? TEXT("true") : TEXT("false"),
												EnumToTCharString(ReasonCode)),
								[this, ReasonCode, bIsSessionPresent]
								{
					...
					for (const bool bIsSessionPresent : {false, true})
					{
						for (const EMqttifyReasonCode ReasonCode : InvalidReasonCodes)
						{
							It(FString::Printf(
								...
								[this, ReasonCode, bIsSessionPresent]
								{
									AddExpectedError(MqttifyReasonCode::InvalidReasonCode,
					...
					{
						It(FString::Printf(TEXT("Should serialize with: Property %s %s"),
											EnumToTCharString(Property.GetIdentifier()),
											*Property.ToString()),
							[this, Property]
							{
								const TArray<uint8> Expected = SerializeConnackMqtt5(true,
																					EMqttifyReasonCode::Banned,
						...
						It(FString::Printf(TEXT("Should deserialize with: Property %s %s"),
											EnumToTCharString(Property.GetIdentifier()),
											*Property.ToString()
							),
							[this, Property]
		...
		Describe("TMqttifyConnAckPacket<EMqttifyProtocolVersion::Mqtt_3>",
				[this]
				{
					for (const bool bIsSessionPresent : {false, true})
					{
						for (uint8 Index = 1; Index < 5; ++Index)
						{
							...
							It(FString::Printf(TEXT("Should serialize with: Session present %s, Return code %s"),
												bIsSessionPresent ? TEXT("true") : TEXT("false"),
