...
namespace Mqttify
{
	...
	{
		Describe("TMqttifyPubCompPacket<EMqttifyProtocolVersion::Mqtt_5>",
				[this]
				{
					It(
					...
					for (const EMqttifyReasonCode ReasonCode : ReasonCodes)
					{
						...
						It(FString::Printf(TEXT("Should deserialize with %s with properties"),
							...
							[this, ReasonCode]
							{
								...
								const TMqttifyPubCompPacket<EMqttifyProtocolVersion::Mqtt_5>
								...
								TestEqual(TEXT("Reason code should be equal"),
										PubCompPacket.GetReasonCode(),
										ReasonCode);

								TestEqual(TEXT("Properties should be equal"),
										PubCompPacket.GetProperties(),
										Properties);
							});
					}
				});

		Describe("TMqttifyPubCompPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>",
				[this]
				{
					...
					It("Should Deserialize success",
						[this]
						{
							...
							const FMqttifyFixedHeader Header = FMqttifyFixedHeader::Create(Reader);
							const TMqttifyPubCompPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>
								PubCompPacket(Reader, Header);
