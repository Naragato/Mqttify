...
namespace Mqttify
{
	...
	void MqttifyDataEncodingDecodingSpec::Define()
	{
		...
		Describe("StringEncodingDecoding",
				[this] {
					...
					It("Should fail gracefully if archive is not in saving mode for EncodeString",
						[this] {
							FArrayReader InArchive{};
							const FString OriginalString = TEXT("MqttifyTest");

							AddExpectedError(Data::ArchiveNotSaving,
											EAutomationExpectedErrorFlags::Contains,
											1);
							Data::EncodeString(OriginalString, InArchive);
						});

					It("Should fail gracefully if archive is not in loading mode for DecodeString",
						[this] {
							...
							AddExpectedError(Data::ArchiveNotLoading,
											EAutomationExpectedErrorFlags::Contains,
											1);
