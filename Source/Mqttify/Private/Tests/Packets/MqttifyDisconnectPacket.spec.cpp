...
namespace Mqttify
{
	BEGIN_DEFINE_SPEC(MqttifyDisconnectPacket,
		...
		static TArray<uint8> SerializeDisconnectMqtt5(EMqttifyReasonCode ReasonCode)
		{
			TArray<uint8> DisconnectPacket = {
				0xE0,                           // Packet type
				2,                              // Size of Reason Code plus 1 byte for Properties Length of 0
	...
	{
		Describe("TMqttifyDisconnectPacket<EMqttifyProtocolVersion::Mqtt_5>",
				[this] {
					for (const EMqttifyReasonCode ReasonCode : ValidReasonCodes)
					{
						It(FString::Printf(
								TEXT("Should serialize with: reason code %s"),
						...
						It(FString::Printf(
								TEXT("Should deserialize with: reason code %s"),
								EnumToTCharString(ReasonCode)),
							[this, ReasonCode] {
								const TArray<uint8> Expected = SerializeDisconnectMqtt5(ReasonCode);
								FArrayReader Reader;
					...
					for (const EMqttifyReasonCode ReasonCode : InvalidReasonCodes)
					{
						...
						It(FString::Printf(
								TEXT(
									"Should log error when deserializing with invalid reason code %s"),
								EnumToTCharString(ReasonCode)),
							[this, ReasonCode] {
								AddExpectedError(MqttifyReasonCode::InvalidReasonCode,
												EAutomationExpectedMessageFlags::Contains);
					...
					for (const FMqttifyProperty Property : ValidProperties)
					{
						It(FString::Printf(
							...
							[this, Property] {
								...
								TMqttifyDisconnectPacket<EMqttifyProtocolVersion::Mqtt_5> DisconnectPacket{
									EMqttifyReasonCode::DisconnectWithWillMessage, FMqttifyProperties{ { Property } } };
							});
		...
		Describe("TMqttifyDisconnectPacket<EMqttifyProtocolVersion::Mqtt_3>",
				[this] {
					It(FString::Printf(
							TEXT("Should serialize")),
						[this] {
