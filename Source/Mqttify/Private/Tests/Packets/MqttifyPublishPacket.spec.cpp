...
BEGIN_DEFINE_SPEC(
		...
		)
	{
		...
		// Topic name 'test'
		const TArray<uint8> TopicName = {0x00, 0x04, 't', 'e', 's', 't'};
		...
		const TArray<uint8> Properties = {0x26, 0x00, 0x04, 'i', 'n', 'f', 'o', 0x00, 0x04, 'd', 'a', 't', 'a'};

		// Payload 'payload'
		const TArray<uint8> Payload = {'p', 'a', 'y', 'l', 'o', 'a', 'd'};

		// Calculate Remaining Length ... we're assuming that this fits in one byte for simplicity
		const int RemainingLength = TopicName.Num() + MessageID.Num() + Properties.Num() + 1 + Payload.Num();
	...
	static TArray<uint8> GetMqtt311PublishPacket(bool bInIsDup, EMqttifyQualityOfService InQoS, bool InShouldRetain)
	{
		...
		// Calculate Remaining Length ... we're assuming that this fits in one byte for simplicity
		const int RemainingLength = TopicName.Num() + MessageID.Num() + Payload.Num();
		checkf(RemainingLength < 256, TEXT("Remaining Length must fit in one byte"));
	...
	Describe(
		...
		[this]
		{
			for (const TTuple<bool, EMqttifyQualityOfService, bool>& PacketType : PacketTypes)
			{
				...
				It(
					FString::Printf(
						TEXT("Should serialize Duplicate %s, QoS %s, Retain %s with properties"),
				...
				It(
					...
					[this, QoS, bIsDup, bShouldRetain]
					{
						...
						TestEqual(TEXT("Is Duplicate should be equal"), PublishPacket.GetIsDuplicate(), bIsDup);

						TestEqual(TEXT("QoS should be equal"), PublishPacket.GetQualityOfService(), QoS);

						TestEqual(TEXT("Retain should be equal"), PublishPacket.GetShouldRetain(), bShouldRetain);

						TestEqual(TEXT("Properties should be equal"), PublishPacket.GetProperties(), Properties);
					});
			}
		});
	...
	Describe(
		...
		[this]
		{
			for (const TTuple<bool, EMqttifyQualityOfService, bool>& PacketType : PacketTypes)
			{
				...
				It(
					FString::Printf(
						TEXT("Should serialize Duplicate %s, QoS %s, Retain %s"),
						BoolToTCharString(bIsDup),
						EnumToTCharString<EMqttifyQualityOfService>(QoS),
						BoolToTCharString(bShouldRetain)),
					[this, bIsDup, QoS, bShouldRetain]
				...
				It(
					...
					[this, QoS, bIsDup, bShouldRetain]
					{
						...
						const FMqttifyFixedHeader Header = FMqttifyFixedHeader::Create(Reader);
						const FMqttifyPublishPacket3 PublishPacket(Reader, Header);
