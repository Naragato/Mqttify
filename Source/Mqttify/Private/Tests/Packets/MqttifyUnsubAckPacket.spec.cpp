...
namespace Mqttify
{
	BEGIN_DEFINE_SPEC(
		...
		static TArray<uint8> GetMqtt5UnsubAckWithProperties(const EMqttifyReasonCode InReasonCode)
		{
			TArray<uint8> UnsubAckPacket = {
				0xB0, // Packet type (0xB0 = UNSUBACK)
		...
		const TArray<uint8> Mqtt3UnsubAck = {
			// Fixed header
			0xB0, // Packet type (0xB0 = UNSUBACK)
			0x02, // Remaining Length (2 bytes for message ID, 1 byte for return code)
	...
	{
		Describe("TMqttifyUnsubAckPacket<EMqttifyProtocolVersion::Mqtt_5>",
				[this]
				{
					It(
					...
					for (const EMqttifyReasonCode ReasonCode : ReasonCodes)
					{
						...
						It(FString::Printf(TEXT("Should deserialize with %s with properties"),
							...
							[this, ReasonCode]
							{
								...
								const TMqttifyUnsubAckPacket<EMqttifyProtocolVersion::Mqtt_5>
								...
								TestEqual(TEXT("Properties should be equal"),
										UnsubAckPacket.GetProperties(),
										Properties);
							});
					}
				});

		Describe("TMqttifyUnsubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>",
				[this]
				{
					It("Should serialize success",
						[this]
						{
							auto UnsubAckPacket = TMqttifyUnsubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>{
								1
							};
					...
					It("Should Deserialize success",
						[this]
						{
							...
							const FMqttifyFixedHeader Header = FMqttifyFixedHeader::Create(Reader);
							const TMqttifyUnsubAckPacket<EMqttifyProtocolVersion::Mqtt_3_1_1>
								UnsubAckPacket(Reader, Header);
